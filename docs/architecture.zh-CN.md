...
### 辐射策略流程
对于多项目生成 (如 "3 个表情包")，通过意图分流处理:

1. **意图分类**: `parse_agent_intent()` 通过 8 条优先级排序的正则规则将 prompt 分类为 multi_item / radiate / iterative / single
2. **multi_item** (表情包/三视图/枚举项): 绕过 Agent Loop，由 `execute_multi_item_intent()` 并行调用 `generate_image` 工具
3. **radiate** (套图/一组/系列): 进入 Agent Loop；LLM 的 `plan` 工具触发 `_execute_radiate()` → 锚点网格 → PIL 裁剪 → 逐项 `chat_edit()` 扩展
4. **iterative**: 进入 Agent Loop；LLM 调用 `plan` 工具（iterative 策略） → `execute_iterative()` 顺序执行
5. **parallel**: 进入 Agent Loop；LLM 调用 `plan` 工具（parallel 策略） → `execute_parallel()` 通过 Semaphore 并发生成

Plan 执行器位于 `backend/app/services/plan_executor.py`。纯后端执行器确保无论 LLM 是否可靠，执行策略一致。
