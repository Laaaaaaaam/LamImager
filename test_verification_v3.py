...
async def run_test(test_id: str, prompt: str, image_count: int = 1, expect_task_type: str = "", expect_strategy: str = ""):
    ...
    try:
        ...
        return {
            "test_id": test_id, "prompt": prompt,
            "task_type": "EXCEPTION", "strategy": "EXCEPTION",
            "steps_summary": [], "images_count": 0,
            "final_images_count": 0, "intermediate_images_count": 0,
            "checkpoint_triggered": False, "pass": False,
...
async def main():
    ...
    results.append(await run_test("V1", "画一只猫", 1, "single", "single"))
    results.append(await run_test("V2", "给我3张不同风格的logo方案", 3, "multi_independent", "parallel"))
    results.append(await run_test("V3", "结合一下这两个logo的设计，生成一个新logo，再细化优化", 1, "iterative", "iterative"))
    results.append(await run_test("V4", "做一套4个橘猫表情包，包含开心、生气、惊讶、害羞", 4, "radiate", "radiate"))
    results.append(await run_test("V5", "做一组统一风格四张图，主题是同一个角色在春夏秋冬四个场景里", 4, "radiate", "radiate"))
    results.append(await run_test("V6", "做一套4个橘猫表情包，包含开心、生气、惊讶、害羞", 4, "radiate", "radiate"))
    results.append(await run_test("V7", "画一只猫", 1, "single", "single"))
